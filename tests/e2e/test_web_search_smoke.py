...
Gated by ``CHANNEL_WEB_SEARCH_ENABLED=1`` AND ``EXA_API_KEY`` present.
...
@pytest.mark.skipif(
    os.environ.get("CHANNEL_WEB_SEARCH_ENABLED") != "1" or not os.environ.get("EXA_API_KEY"),
    reason="requires CHANNEL_WEB_SEARCH_ENABLED=1 AND EXA_API_KEY set",
)
