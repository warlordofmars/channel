...
| `CHAT#{chat_id}` | `SUMMARY` | Rolling head summary — the gist of turns that scrolled out of the token-budgeted history window | no | At most one row per chat (#245). Carries `text` (≤ 4 000 chars), `covers_through` (the full `MSG#{created_at}#{msg_id}` key of the newest turn folded in — the resume point, advanced only when a pass persists) and `updated_at`. The SK carries no `MSG#` prefix, so every existing `begins_with("MSG#")` query skips it unmodified; `delete_chat` deletes it explicitly. Sorts *after* the `MSG#` rows (`M` < `S`) — immaterial, only the prefix disjointness is load-bearing. |
